---
title: 'WGCNA: Megachile rotundata Maternal Effects Project: Egg Samples'
author: "M.A. Hagadorn"
...
So, first we load in the egg specific expression data (that has been cleaned, i.e. genes removed).
```{r loadEGGData, echo=FALSE}
load("data/Mrotundata_expdataClean_EGG.RData")
```

# Step One: cluster samples
## EGG SAMPLES: Cluster sample
s  to determine if there are obvious outliers
```{r clustersamples}
#create a distance matrix and do a hierachical cluster analysis
#using data set with genes removed
sampleTree_precut <- hclust(dist(expd_EGG), method = "average")
...
#cutoffline to plot
abline(h=160000, col='red')

###
###
###
#Remove outliers
#Identify the cluster that falls below the cutoff we set
samplecluster <- cutreeStatic(sampleTree_precut, cutHeight=160000, minSize=10)
#cutHeight=height at which branches are cut
#minSize=minimum number of objects on a branch to be considered a cluster
#output=1 or 0; 1=falls below the line so good while 0 = samples that fall above the line

table(samplecluster)

keepSamples <- (samplecluster==1) #keep the samples that are below the threshold
expdatEGG <- expd_EGG[keepSamples, ] #only samples that met threshold
nGenes <- ncol(expdatEGG)
nSamples <-nrow(expdatEGG)


#expdatEGG==new variable ready for expression network analysis
...
sampleData <- sampleData_all[sampleData_all$oocyte_egg=="egg",-c(4,7,8)]
dim(sampleData)
names(sampleData)


#Make new dataframe analogous to expression data that will hold the traits
samples <- rownames(expdatEGG) #save names of all rows in variable expdatEGG_cleaned
variableRows <- match(samples,sampleData$id) #returns vector of positions for these

varData <- sampleData[variableRows, -1] #make new dataframe with Mice removed but positions as rows
...
##expression data w/ genes and outliers removed==stored as expdatEGG
##Sample variables==stored as VarData


#re-cluster samples (because we removed the one sample)
sampleTree_outlierTrim <- hclust(dist(expdatEGG), method="average")
...
#plot
plotDendroAndColors(sampleTree_outlierTrim, varColors,  #Sample tree with all samples
...
save(expdatEGG, varData, file="Mrotundata_EGG_step1_expression&variablesetup.RData")
```
...
softThres <- pickSoftThreshold(expdatEGG, powerVector = powers, verbose = 5)
...
adjacency <- adjacency(expdatEGG, power=softPower) 
```
...
ModuleEigenLs <- moduleEigengenes(expdatEGG, colors = dynamicColors)
...
#No values for gray, so removing for cluster visualization
...
#This corresponds to a similarity of .75 to merge a module
MEDissThres <- 0.25
...
mergeModules <- mergeCloseModules(expdatEGG, dynamicColors, cutHeight = MEDissThres, verbose = 3) #cutoff of .25
...
save(MEs, moduleLabels, moduleColors, geneTree, TOM, dissTOM, 
     file = "Mrotundata_EGG_step2_ModuleAssignment&Merge.RData") 
```
...
MEs_modVar <- moduleEigengenes(expdatEGG, moduleColors)$eigengenes
...
labeledHeatmap(Matrix=moduleVarCor,
               ...
               cex.text = 0.7,
               zlim = c(-1,1),
               main = "EGG TISSUE: Module-variable relationships cutHeight = 0.25")
...
geneModuleMembership <- as.data.frame(cor(expdatEGG, MEs_modVar, use = "p"))
...
geneVarSignificance_tx <- as.data.frame(cor(expdatEGG, tx, use = "p"))
...
geneVarSignificance_wave <- as.data.frame(cor(expdatEGG, wave, use = "p"))
...
No modules are significant by factor, so no intramodular analysis done.
...
geneInfo_raw <- data.frame(geneid = names(expdatEGG),
...
geneInfo_EGG = geneInfo_raw[geneOrder, ]
...
modulecounts <- count(geneInfo_EGG, 'moduleColor')
...
barplot2(modulecounts_ordered$freq, horiz = TRUE, names.arg = 
           ...
           col=modulecounts_ordered$moduleColor, xlim = c(0,4500), xaxt="n")
axis(1, pos=0, at = seq(0,4500,500)) # Draw x-axis at 0 line
...
```{r geneinfo_CSV}
# Save module colors and labels for use in subsequent parts
save(geneModuleMembership, geneVarSignificance_tx, geneVarSignificance_wave, GSPvalue_tx, GSPvalue_wave, wave, tx, file = "Mrotundata_EGG_step3_ModuleMembership&GeneInfoID.RData")



write.csv(geneInfo_EGG, file = "geneInfo_EGG.csv")
```
...
###RENAME FILES CONTAINING THE EGG LABEL###
#Makes life easier in the module overlap
geneModuleMembership_EGG <- geneModuleMembership
MEs_EGG <- MEs
moduleColors_EGG <- moduleColors
moduleLabels_EGG <- moduleLabels


# Save module colors and labels for use in subsequent parts
save(geneModuleMembership_EGG, moduleColors_EGG, moduleLabels_EGG, MEs_EGG, file = "Mrotundata_EGG_ModuleOverlapData.RData")
```
